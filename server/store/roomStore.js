...
const addPublicRoom = (roomId, userCount) => {
    const roomExists = publicRoomStore.find(room => room.roomId == roomId);
    if (roomExists) {
        return { success: false, message: "Room Already exists. Please try another Room ID" }
    } else {
        const newRoom = {
            roomId, participants: [], userCount
        }
...
const addPrivateRoom = (roomId, password, userCount) => {
    const roomExists = privateRoomStore.find(room => room.roomId == roomId);
    if (roomExists) {
        return { success: false, message: "Room Already exists. Please try another Room ID" }
    } else {
        const newRoom = {
            roomId, participants: [], password, userCount
        }
    ...
    const result = privateRoomStore.filter(room => {
        if (room.roomId === roomId) {
            if (room.password === password) {
                const hasParticipant = room.participants.find((user) => user.id == userId);
                if (!hasParticipant) {
                    room.participants.push({ id: userId, name: username });
                    room.userCount += 1;
                    return { success: true, room }
                } else {
                    return { success: false, message: 'User is already added to the room.' }
                }
            } else {
...
const removePrivateRoomParticipants = (roomId, userId) => {
    const roomIndex = privateRoomStore.findIndex(room => room.roomId === roomId);
    if (privateRoomStore[roomIndex].userCount !== 0 && privateRoomStore[roomIndex].participants.find(participant => participant.id === userId)) {
        const participantIndex = privateRoomStore[roomIndex].participants.findIndex(user => user.id === userId);
...
const removePublicRoomParticipants = (roomId, userId) => {
    ...
    if (publicRoomStore[roomIndex].userCount !== 0 && publicRoomStore[roomIndex].participants.find(participant => participant.id === userId)) {
        const participantIndex = publicRoomStore[roomIndex].participants.findIndex(user => user.id === userId);
...
export { generatePrivateRoomId, generatePublicRoomId, addPrivateRoom, addPublicRoom, addPrivateRoomParticipants, addPublicRoomParticipants, removePrivateRoom, removePublicRoom, removePrivateRoomParticipants, removePublicRoomParticipants, getRoom }