...
import { addPrivateRoomParticipants, addPublicRoomParticipants, removePrivateRoomParticipants, removePublicRoomParticipants } from './store/roomStore.js';
...
io.on('connection', (socket) => {
    ...
    if (!activeSocket) {
        activeSockets.push({ id: socket.id, joined: false, roomId: null, secureRoom: false });
    }

    socket.on('join-room', ({ roomId, username, password, secureRoom }) => {
        console.log(`Room Join Request - RoomID: ${roomId}, ${socket.id}, ${username}`);
        const addRoomParticipants = (roomId, userId, username, password) => secureRoom ? addPrivateRoomParticipants(roomId, userId, username, password) : addPublicRoomParticipants(roomId, userId, username);
        ...
        if (!room.roomId) {
            ...
            if (userActiveSocket && userActiveSocket.joined === false) {
                ...
                if (room.participants) {
                    const peers = room.participants.map((peer) => {if(!currentParticipants.includes(peer.id)) return peer.id});
    ...
    socket.on("offer", (payload) => {
        io.to(payload.userToSignal).emit('user-connected', { signal: payload.signal, callerID: payload.callerID });
    });
